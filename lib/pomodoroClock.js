...
function startCounter(doc, win, sessionLength, breakLength) {
    ...
    */
    function updateTime() {
        doc.getElementById("session").innerHTML.disabled = true;
        if ((global.sessionTime + global.breakTime) >= 0) {
            if (global.sessionTime >= 0) {
                global.sessionTime -= 1;
                doc.getElementById("timer").innerHTML = timeToString(global.sessionTime);
            } else {
                global.breakTime -= 1;
                doc.getElementById("timer").innerHTML = timeToString(global.breakTime);
            }
            const tm = win.setTimeout(updateTime, COUNT_DOWN_TIME);

            return tm;
        }
        //jQuery("#session").disabled = false;
        return null;
    }
