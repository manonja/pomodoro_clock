...
const Clock = {
    ...
    start(sessionLength, breakLength) {
        ...
        if (this.originSessionTime === 0 && this.originBreakTime === 0) {
            this.originSessionTime = this.sessionTime;
            this.originBreakTime = this.breakTime;
        }

        // document.getElementById("timer").onclick = Clock.pause();
        // $("#timer").click(() => {
        ...
        this.interval = setInterval(() => {
            if ((self.sessionTime + self.breakTime) >= 1) {
                if (self.sessionTime >= 1) {
                    ...
                    // TODO: make sure there is no bug -1 between session and break
                    self.breakTime -= 1;
